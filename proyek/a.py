...
def laporan():
    ...
    tabs = st.tabs(["Ringkasan", "Tren Harian", "Jurnal Umum", "Buku Besar", "Laba Rugi", "Neraca", "Kelola Transaksi"])
    ...
    with tabs[0]:
        ...
        if total_pemasukan > 0 or total_pengeluaran > 0:
            ...
            fig = px.pie(df_sum, values='Jumlah', names='Kategori', title='Proporsi Pemasukan vs Pengeluaran')
            st.plotly_chart(fig, use_container_width=True)

    with tabs[1]:
        st.subheader("Tren Pemasukan dan Pengeluaran")
        
        freq = st.radio("Grup Berdasarkan:", ["Harian", "Mingguan", "Bulanan"], horizontal=True)
        freq_map = {"Harian": "D", "Mingguan": "W", "Bulanan": "M"}
        
        if not pemasukan_df.empty:
            pemasukan_agg = pemasukan_df.resample(freq_map[freq], on='Tanggal').sum(numeric_only=True).reset_index()
        else:
            pemasukan_agg = pd.DataFrame(columns=["Tanggal", "Jumlah"])
            
        if not pengeluaran_df.empty:
            pengeluaran_agg = pengeluaran_df.resample(freq_map[freq], on='Tanggal').sum(numeric_only=True).reset_index()
        else:
            pengeluaran_agg = pd.DataFrame(columns=["Tanggal", "Jumlah"])
        
        fig = px.line(title=f'Tren {freq} Pemasukan dan Pengeluaran')
        
        if not pemasukan_agg.empty:
            fig.add_scatter(x=pemasukan_agg['Tanggal'], y=pemasukan_agg['Jumlah'], 
                          mode='lines+markers', name='Pemasukan', line=dict(color='green'))
        
        if not pengeluaran_agg.empty:
            fig.add_scatter(x=pengeluaran_agg['Tanggal'], y=pengeluaran_agg['Jumlah'], 
                          mode='lines+markers', name='Pengeluaran', line=dict(color='red'))
        ...
        with col1:
            st.write("Pemasukan")
            if not pemasukan_agg.empty:
                st.dataframe(pemasukan_agg)
            else:
                st.info("Tidak ada data pemasukan")
        with col2:
            st.write("Pengeluaran")
            if not pengeluaran_agg.empty:
                st.dataframe(pengeluaran_agg)
            else:
    ...
    with tabs[6]:
        ...
        with col2:
            ...
            if not pengeluaran_df.empty:
                pengeluaran_df['No'] = pengeluaran_df.index + 1
                st.dataframe(pengeluaran_df[['No', 'Tanggal', 'Kategori', 'Sub Kategori', 'Jumlah', 'Keterangan']])
                
                selected_pengeluaran = st.selectbox(
                    "Pilih Pengeluaran untuk Dihapus",
                    options=pengeluaran_df.index,
                    format_func=lambda x: f"{pengeluaran_df.loc[x, 'Tanggal']} - {pengeluaran_df.loc[x, 'Kategori']} - Rp {pengeluaran_df.loc[x, 'Jumlah']:,}"
                )
                
                if st.button("Hapus Pengeluaran Terpilih"):
                    if hapus_transaksi("pengeluaran", selected_pengeluaran, username):
                        st.success("Pengeluaran berhasil dihapus dan jurnal pembalikan telah dibuat.")
                        st.rerun()
                    else:
                        st.error("Gagal menghapus pengeluaran.")
            else:
                st.info("Tidak ada data pengeluaran.")
...
def main():
    ...
    st.image('proyek/aset/Logo.png', width=5000)
